...
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/vKhonshu/intro2/main/ui2"))()
    ...
    NotifyLib.prompt('uzisamaware  - made by uzi#3737', 'Loading..')
     ...
     while true do
        ...
        for i,v in pairs(char:GetChildren()) do
           if v:IsA("Script") and v:FindFirstChildOfClass("LocalScript") then
              v:FindFirstChildOfClass("LocalScript").Source = "Cleared by uzisamaware"
              return
           end
        end
     end
                ...
                if getgenv().loltoggle then
                    ...
                    if Locking then
                        ...
                        if getgenv().ChatMode == true then
                            local A_3 = "uzisamaware: "..Plr.DisplayName
                            local A_4 = "All"
...
local window = library:new({textsize = 13.5,font = Enum.Font.RobotoMono,name = "uzisamaware",color = Color3.fromRGB(204, 255, 255)})
...
section1:button({name = "uzisamaware",callback = function()
    if game.PlaceId == 2788229376 then
        print'da hood'

    NotifyLib.prompt('Game Detected: Da Hood', 'uzi#3737')
            Settings = {
            rewrittenmain = {
            Enabled = true,
            Key = "q",
    ...
    NotifyLib.prompt('Game Detected: Hood Modded', 'uzi#3737')
            Settings = {
            rewrittenmain = {
            Enabled = true,
            Key = "q",
...
function getClosestPlayerToCursor()
   ...
   return closestPlayer
    ...
    if game.PlaceId == 9183932460 then
        print'Untitled Hood'
    NotifyLib.prompt('Game Detected: Untitled Hood', 'uzi#3737')
            Settings = {
            rewrittenmain = {
            Enabled = true,
            Key = "q",
